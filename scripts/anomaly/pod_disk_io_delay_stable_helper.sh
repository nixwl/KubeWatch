...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/disk/delay/stable"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/disk-io-payload-template.pod.delay.stable.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/disk-io-payload-env.pod.delay.stable.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/disk-io-payload-script.pod.delay.stable.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/disk-io-payload.pod.delay.stable.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/disk-io-payload.pod.delay.stable.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-disk-io-payload-env-pod-delay-stable-configmap}"
CRON_NAME="${CRON_NAME:-disk-io-payload-pod-delay-stable-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-disk-io-payload-pod-delay-stable}"
...
Quick start (recommended):
  ...
  # $0 set LOG_DIR=/logs CLEANUP=true BLADE_NAME=chaosblade-pod-io-delay-stable-payload
...
# patch env CM：./pod_disk_delay_stable_helper.sh set IO_DELAY_MS=1000 ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
