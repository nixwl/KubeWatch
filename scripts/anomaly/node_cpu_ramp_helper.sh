...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/node/cpu/ramp"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/cpu-payload-template.ramp.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/cpu-payload-env.ramp.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/cpu-payload-script.ramp.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/cpu-payload.ramp.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/cpu-payload.ramp.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-cpu-payload-env-ramp-configmap}"
CRON_NAME="${CRON_NAME:-cpu-singlenode-ramp-payload-jitter}"
JOB_APP_LABEL="${JOB_APP_LABEL:-cpu-singlenode-ramp-payload-jitter}"
...
Quick start (recommended):
  ...
  $0 set TARGET_NODE=emanage1 HIGH=60 LOW=10 ...
  $0 set RAMP_SECONDS=60 HOLD_SECONDS=600 ...
...
# patch env CM：./node_cpu_ramp_helper.sh set HIGH=60 LOW=10 ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
case "$cmd" in
  ...
  install)
    # Desired semantics (per your request):
  ...
  show)
    ...
    kubectl -n "$NS" get cm "$ENV_CM_NAME" -o yaml 2>/dev/null | sed -n '1,160p' || true
    echo
