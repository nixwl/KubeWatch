...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/self-resource/pod-delete"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/pod-payload-template.pod.delete.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/pod-payload-env.pod.delete.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/pod-payload-script.pod.delete.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/pod-payload.pod.delete.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/pod-payload.pod.delete.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-pod-payload-env-pod-delete-configmap}"
CRON_NAME="${CRON_NAME:-pod-payload-pod-delete-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-pod-payload-pod-delete}"
...
Quick start (recommended):
  ...
  # NOTE: POD_NAME and POD_LABELS => set at least one
  ...
  # $0 set EVICT_COUNT=1 WAITING_TIME=20s
  # $0 set EVICT_PERCENT=50 WAITING_TIME=20s

  # runtime control
  $0 set DELETE_MIN_HOLD_SECONDS=60 DELETE_MAX_HOLD_SECONDS=120
  $0 set DELETE_TIMEOUT_PADDING=15
  # optional:
  # $0 set DELETE_HOLD_SECONDS=90           # fixed hold (bypass random [min,max])
  # $0 set DELETE_ENABLE_TIMEOUT=false      # recommended false for delete(one-shot)
  # $0 set BLADE_NAME=chaosblade-pod-delete-payload CLEANUP=true LOG_DIR=/logs
...
# patch env CM：./pod_self_resource_delete_helper.sh set TARGET_NS=default POD_NAME=xxx ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
