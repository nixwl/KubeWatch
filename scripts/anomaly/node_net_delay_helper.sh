...
# ========= basic =========
DIR="${DIR:-/data/nfs-conf/chaosblade/job/node/net/delay}"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/net-payload-template.delay.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/net-payload-env.delay.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/net-payload-script.delay.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/net-payload.delay.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/net-payload.delay.job.yaml}"

# From your delay YAMLs:
ENV_CM_NAME="${ENV_CM_NAME:-net-payload-env-delay-configmap}"
CRON_NAME="${CRON_NAME:-net-payload-delay-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-net-payload-delay}"
...
Install modes (split responsibilities):
  ...
  $0 install cron            # install CronJob resources only (SA/CR/CRB/CronJob...) - NO ConfigMaps/NO Job prereqs
  $0 install all             # install ConfigMaps + Job prerequisites + CronJob resources
  $0 uninstall               # delete CronJob resources + Job prerequisites + ConfigMaps (best-effort)
...
Ops:
  ...
  $0 stop                               # delete current ChaosBlade CR by BLADE_NAME in env configmap
...
Env keys (from net-payload-env.delay.configmap.yaml):
  TARGET_NODE=<nodeName>              # target node name (template matcher: names)
  BLADE_NAME=<string>                 # ChaosBlade instance name
  CLEANUP=true|false                  # cleanup ChaosBlade CR at end

  TIME_MS=<uint>                      # delay time (ms)
  OFFSET_MS=<uint>                    # delay jitter offset (ms)
  INTERFACE=<ifname>                  # e.g. eth0/ens33

  # optional filters (unset/'-' => removed matcher)
  LOCAL_PORT=-
  REMOTE_PORT=-
  DESTINATION_IP=-
  EXCLUDE_PORT=-
  ...
  # optional flags (not true => removed matcher)
  FORCE=true|false
  IGNORE_PEER_PORT=true|false

  # runtime control
  MIN_HOLD_SECONDS=<uint>
  MAX_HOLD_SECONDS=<uint>
  HOLD_SECONDS=<uint>                 # optional fixed hold seconds (if set, overrides min/max)
  TIMEOUT_PADDING=<uint>              # timeout = hold + padding
  LOG_DIR=/logs

Important constraints:
  - EXCLUDE_PORT is mutually exclusive with LOCAL_PORT/REMOTE_PORT.
    If you set LOCAL_PORT/REMOTE_PORT, set EXCLUDE_PORT='-'.
...
Recommended workflow:
  ...
  $0 set TARGET_NODE=emanage1 TIME_MS=150 OFFSET_MS=50 INTERFACE=ens33
  $0 set EXCLUDE_PORT=22 MIN_HOLD_SECONDS=600 MAX_HOLD_SECONDS=1200 TIMEOUT_PADDING=30 CLEANUP=true

  # 3) install job prerequisites + run once
  $0 install job
  $0 job run
  $0 job logs last

  # 4) install cron resources only (ensure configmaps already exist)
  $0 install cron
  $0 cron set schedule="0 * * * *" activeDeadlineSeconds=1800
  $0 cron resume
  $0 cron enable
EOF
}
...
Subcmd:
  enable                   Apply cron YAML (create/update CronJob resources)
...
# patch env CM: ./node_net_delay_helper.sh set TARGET_NODE=... TIME_MS=...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
cron_set() {
  ...
  for kv in "$@"; do
    ...
    case "$ku" in
      ...
      *) echo "ERROR: unknown cron field: $k"; exit 1 ;;
    esac
  done
...
job_set() {
  ...
  local backoffLimit="" activeDeadlineSeconds="" ttlSecondsAfterFinished="" parallelism="" completions=""
  local kv k v ku
  for kv in "$@"; do
    [[ "$kv" == *"="* ]] || { echo "ERROR: invalid kv: $kv (need KEY=VALUE)"; exit 1; }
    k="${kv%%=*}"; v="${kv#*=}"
...
case "$cmd" in
  ...
  install)
    ...
    case "$mode" in
      ...
      job)  apply_job_prereqs; echo "Job prerequisites installed. (No ConfigMaps, no CronJob.)" ;;
      cron) apply_cron; echo "CronJob resources installed. (No ConfigMaps, no Job prerequisites.)" ;;
      all)  apply_configmaps; apply_job_prereqs; apply_cron; echo "All installed: ConfigMaps + Job prereqs + Cron resources." ;;
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "Example: $0 set TARGET_NODE=emanage1 TIME_MS=150 OFFSET_MS=50 INTERFACE=ens33"
      exit 0
    fi
  ...
  show)
    ...
    kubectl -n "$NS" get cm "$ENV_CM_NAME" -o yaml 2>/dev/null | sed -n '1,260p' || echo "(no env configmap)"
    echo
    echo "=== cronjob ($NS/$CRON_NAME) ==="
