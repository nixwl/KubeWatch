...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/cpu/ramp"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/cpu-payload-template.pod.ramp.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/cpu-payload-env.pod.ramp.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/cpu-payload-script.pod.ramp.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/cpu-payload.pod.ramp.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/cpu-payload.pod.ramp.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-cpu-payload-env-pod-ramp-configmap}"
CRON_NAME="${CRON_NAME:-cpu-payload-pod-ramp-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-cpu-payload-pod-ramp}"
...
Usage:
  ...
  $0 install cron            # install CronJob only - NO ConfigMaps/NO Job prerequisites
  $0 install all             # install ConfigMaps + Job prerequisites + CronJob

  $0 uninstall               # delete CronJob + Jobs + Pods + Job prerequisites + ConfigMaps (best-effort)

  $0 cron  <subcmd> [args...]
  $0 job   <subcmd> [args...]

  $0 set   KEY=VALUE [KEY=VALUE ...]    # patch env configmap data
  $0 select <label-selector> [namespace]  # Randomly select a Pod by label selector and auto-configure it (new feature)
...
Quick start (recommended):
  ...
  # Method B: automatically select a Pod via label selector (supports Deployment/StatefulSet, etc.)
  $0 select "app=nginx" default
  $0 set HIGH=90 LOW=80 RAMP_SECONDS=15 HOLD_SECONDS=25 CYCLES=5
...
Label selector examples:
  # Select a random Pod from a Deployment
  $0 select "app.kubernetes.io/name=zot" zot
  
  # Select a specific Pod from a StatefulSet
  $0 select "statefulset.kubernetes.io/pod-name=mysql8-primary-0" mysql-namespace
  
  # Select all Pods with a specific label
  $0 select "app=myapp,env=prod" production
...
Subcmd:
  enable                   Apply cron YAML (create/update CronJob)
...
apply_cron() {
  echo "Apply CronJob from $CRON_YAML ..."
  kapply "$CRON_YAML"
}
...
# Patch env CM: ./pod_cpu_ramp_helper.sh set KEY=VALUE ...
...
cm_set() {
    ...
    if [[ -n "$selector" && -z "$(echo "${new_args[@]}" | grep -o 'POD_NAME=')" ]]; then
        ...
        echo "Using label selector: $selector, namespace: $target_ns, selection mode: $selector_mode" >&2
        local pod_info
        ...
        if [[ -n "$pod_info" ]]; then
            ...
            echo "Selected Pod: $pod_name, container: $container_name" >&2
    ...
    echo "Updating ConfigMap: $NS/$ENV_CM_NAME" >&2
...
# ========= New command: select Pods via label selector =========
# Usage: $0 select <label-selector> [namespace] [mode]
# Examples:
#   $0 select "app.kubernetes.io/name=zot" zot
#   $0 select "statefulset.kubernetes.io/pod-name=mysql8-primary-0" mysql-namespace
select_pod_by_label() {
    ...
    if [[ -z "$namespace" ]]; then
        ...
        if [[ -z "$namespace" ]]; then
            namespace="default"
            echo "Namespace not specified; using default value: $namespace" >&2
        else
            echo "Using TARGET_NS from the ConfigMap: $namespace" >&2
        fi
    fi
    
    echo "Selecting Pod via label selector..." >&2
    echo "Selector: $selector" >&2
    echo "Namespace: $namespace" >&2
    echo "Selection mode: $mode" >&2
    
    # Validate that the selector matches at least one Pod
    local pod_count
    pod_count=$(kubectl -n "$namespace" get pods -l "$selector" -o name 2>/dev/null | wc -l)
    
    if [[ "$pod_count" -eq 0 ]]; then
        echo "ERROR: no Pod found for label '$selector' in namespace '$namespace'" >&2
        echo "Hint: use 'kubectl -n $namespace get pods --show-labels' to view available labels" >&2
        exit 1
    fi
    
    echo "Found $pod_count matching Pods" >&2
    ...
    if [[ -n "$pod_info" ]]; then
        ...
        echo "✓ Successfully selected Pod: $pod_name" >&2
        echo "✓ Container name: $container_name" >&2
        
        # Automatically update the ConfigMap
        echo "Updating ConfigMap..." >&2
        ./pod_cpu_ramp_helper.sh set TARGET_NS="$namespace" POD_NAME="$pod_name" CONTAINER_NAME="$container_name"
        
        echo "✓ Updated ConfigMap: TARGET_NS=$namespace, POD_NAME=$pod_name, CONTAINER_NAME=$container_name" >&2
        echo "" >&2
        echo "You can now use the following command to set other parameters:" >&2
        echo "  $0 set HIGH=90 LOW=80 RAMP_SECONDS=15" >&2
    else
        echo "ERROR: unable to select a Pod" >&2
        exit 1
    fi
...
case "$cmd" in
  ...
  uninstall)
    echo "Delete CronJob (if any)..."
    # delete by yaml and by name (double insurance)
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo ""
      echo "Examples:"
      echo "  $0 set TARGET_NS=default POD_NAME=app-1 CONTAINER_NAME=app"
      echo "  $0 set TARGET_NS=zot SELECTOR='app.kubernetes.io/name=zot' HIGH=90"
      echo "  $0 set TARGET_NS=mysql SELECTOR='statefulset.kubernetes.io/pod-name=mysql8-primary-0' SELECTOR_MODE=first"
      exit 0
    fi
  ...
  select)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo ""
      echo "Purpose: select a Pod via label selector and automatically update the ConfigMap"
      echo ""
      echo "Parameters:"
      echo "  <label-selector>  Label selector, e.g. 'app=nginx' or 'app.kubernetes.io/name=zot'"
      echo "  [namespace]       Namespace (optional; defaults to ConfigMap or 'default')"
      echo "  [mode]           Selection mode: random=random selection, first=select the first (default: random)"
      echo ""
      echo "Examples:"
      echo "  # Randomly select a Zot Pod"
      echo "  $0 select \"app.kubernetes.io/name=zot\" zot"
      echo ""
      echo "  # Select a specific StatefulSet Pod"
      echo "  $0 select \"statefulset.kubernetes.io/pod-name=mysql8-primary-0\" mysql-namespace first"
      echo ""
      echo "  # Select the first matching Pod"
      echo "  $0 select \"app=myapp\" production first"
      exit 0
    fi
