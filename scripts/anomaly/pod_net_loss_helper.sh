...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/net/loss"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/net-payload-template.pod.loss.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/net-payload-env.pod.loss.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/net-payload-script.pod.loss.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/net-payload.pod.loss.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/net-payload.pod.loss.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-net-payload-env-pod-loss-configmap}"
CRON_NAME="${CRON_NAME:-net-payload-pod-loss-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-net-payload-pod-loss}"
...
Quick start (recommended):
  ...
  # NOTE: optional fields can be "-" meaning empty; run.sh will normalize "-" -> ""
  $0 set TARGET_NS=default
  $0 set POD_NAME=xxx
  # OR:
  # $0 set POD_LABELS="app=stress-test"

  # loss params
  $0 set LOSS_PERCENT=30 INTERFACE=eth0
  # optional filters
  # $0 set LOSS_DESTINATION_IP=10.0.0.1 LOSS_EXCLUDE_IP=10.0.0.2
  # $0 set LOSS_LOCAL_PORT=8080 LOSS_REMOTE_PORT=443 LOSS_EXCLUDE_PORT=8443

  # optional flags
  # $0 set LOSS_FORCE=false LOSS_IGNORE_PEER_PORT=false

  # runtime control
  $0 set LOSS_MIN_HOLD_SECONDS=600 LOSS_MAX_HOLD_SECONDS=1200
  $0 set LOSS_TIMEOUT_PADDING=30
  # optional:
  # $0 set LOSS_HOLD_SECONDS=900         # fixed hold (bypass random [min,max])
  # $0 set WAITING_TIME=20s EVICT_COUNT=1 EVICT_PERCENT=50
...
# patch env CM：./pod_net_loss_helper.sh set LOSS_PERCENT=30 ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
