...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/disk/error/intermit"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/disk-io-payload-template.pod.error.intermit.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/disk-io-payload-env.pod.error.intermit.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/disk-io-payload-script.pod.error.intermit.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/disk-io-payload.pod.error.intermit.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/disk-io-payload.pod.error.intermit.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-disk-io-payload-env-pod-error-intermit-configmap}"
CRON_NAME="${CRON_NAME:-disk-io-payload-pod-error-intermit-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-disk-io-payload-pod-error-intermit}"
...
Quick start (recommended):
  ...
  # OR:
  # $0 set TARGET_NS=default POD_LABELS="app=stress-test"

  # IO error params
  $0 set IO_METHOD=read IO_ERRNO=5 IO_PERCENT=100 IO_PATH=/data/conf

  # intermit windows
  $0 set IO_FAIL_MIN_HOLD_SECONDS=30 IO_FAIL_MAX_HOLD_SECONDS=120
  $0 set IO_RECOVER_MIN_SECONDS=30 IO_RECOVER_MAX_SECONDS=180

  # stop condition (choose ONE)
  $0 set IO_TOTAL_SECONDS=600
  # OR:
  # $0 set IO_CYCLES=10

  # misc
  $0 set IO_TIMEOUT_PADDING=30 CLEANUP=true LOG_DIR=/logs
  # optional:
  # $0 set EVICT_COUNT=1 EVICT_PERCENT=50 WAITING_TIME=20s

  # optional: generate IO inside container (to make the failure observable)
  # $0 set IO_GEN_ENABLE=true IO_GEN_MODE=mixed IO_GEN_INTERVAL_MS=200 IO_GEN_FILE_KB=64 IO_GEN_FILENAME=seed.bin
  # $0 set IO_GEN_CONTAINER=app
...
# patch env CM：./pod_disk_error_intermit_helper.sh set IO_METHOD=read ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
