...
# ========= basic =========
DIR="${DIR:-/data/nfs-conf/chaosblade/job/node/disk/fill}"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/disk-payload-template.fill.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/disk-payload-env.fill.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/disk-payload-script.fill.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/disk-payload.fill.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/disk-payload.fill.job.yaml}"

# From your fill YAMLs:
ENV_CM_NAME="${ENV_CM_NAME:-disk-payload-env-fill-configmap}"
CRON_NAME="${CRON_NAME:-disk-payload-fill-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-disk-payload-fill}"
...
Env keys (from disk-payload-env.fill.configmap.yaml):
  TARGET_NODE=<nodeName>
  BLADE_NAME=<string>
  CLEANUP=true|false

  # required
  DISK_PATH=<path>              # target path on node filesystem (DO NOT use env var named PATH!)

  # choose ONE (mutually exclusive)
  PERCENT=<0..100 int>          # fill to percent usage
  RESERVE_MB=<MB int>           # keep at least this much free
  SIZE_MB=<MB int>              # fill fixed amount

  # optional flag
  RETAIN_HANDLE=true|false
...
Recommended workflow:
  ...
  $0 set TARGET_NODE=emanage1 DISK_PATH=/var/lib/rancher PERCENT=90
  # OR: $0 set RESERVE_MB=10240 PERCENT=- SIZE_MB=-
  # OR: $0 set SIZE_MB=2048 PERCENT=- RESERVE_MB=-
  $0 set MIN_HOLD_SECONDS=900 MAX_HOLD_SECONDS=1800 TIMEOUT_PADDING=30 RETAIN_HANDLE=false CLEANUP=true
  ...
  $0 install cron
  $0 cron set schedule="0 * * * *" activeDeadlineSeconds=2100
  $0 cron resume
  $0 cron enable

Notes:
  - Job status/logs filter Jobs by label: app=${JOB_APP_LABEL} (manual runs).
  - CronJob-created Jobs are best inspected via: $0 cron status (label cronjob-name=${CRON_NAME})
EOF
}
...
# patch env CM: ./node_disk_fill_helper.sh set TARGET_NODE=... DISK_PATH=... PERCENT=...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
case "$cmd" in
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "Example: $0 set TARGET_NODE=emanage1 DISK_PATH=/var/lib/rancher PERCENT=90 RESERVE_MB=- SIZE_MB=-"
      exit 0
    fi
  ...
  show)
    ...
    kubectl -n "$NS" get cm "$ENV_CM_NAME" -o yaml 2>/dev/null | sed -n '1,280p' || echo "(no env configmap)"
    echo
    echo "=== cronjob ($NS/$CRON_NAME) ==="
