...
# ========= basic =========
DIR="${DIR:-/data/nfs-conf/chaosblade/job/node/net/loss}"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/net-payload-template.loss.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/net-payload-env.loss.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/net-payload-script.loss.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/net-payload.loss.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/net-payload.loss.job.yaml}"

# From your net loss YAMLs:
ENV_CM_NAME="${ENV_CM_NAME:-net-payload-env-loss-configmap}"
CRON_NAME="${CRON_NAME:-net-payload-loss-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-net-payload-loss}"
...
Env keys (from net-payload-env.loss.configmap.yaml):
  ...
  PERCENT=<int>                       # loss percent [0..100]
  INTERFACE=<ifname>                  # e.g. eth0/ens33

  # optional filters (empty/'-' means: not set; script will remove the matcher)
  LOCAL_PORT=-
  REMOTE_PORT=-
  DESTINATION_IP=-
  EXCLUDE_IP=-
  ...
  # IMPORTANT: exclude-port cannot be used together with local-port/remote-port.
...
Recommended workflow:
  ...
  $0 set TARGET_NODE=emanage1 PERCENT=10 INTERFACE=ens33
...
# patch env CM: ./node_net_loss_helper.sh set TARGET_NODE=... PERCENT=...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
case "$cmd" in
  ...
  install)
    ...
    #   - install cron   => CronJob resources only
    #   - install all    => ConfigMaps + Job prerequisites + CronJob resources
    mode="${1:-cm}"
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "Example: $0 set TARGET_NODE=emanage1 PERCENT=10 INTERFACE=ens33 EXCLUDE_PORT=22"
      exit 0
    fi
