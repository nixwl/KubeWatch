...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/disk/error/fixed"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/disk-io-payload-template.pod.error.fixed.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/disk-io-payload-env.pod.error.fixed.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/disk-io-payload-script.pod.error.fixed.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/disk-io-payload.pod.error.fixed.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/disk-io-payload.pod.error.fixed.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-disk-io-payload-env-pod-error-fixed-configmap}"
CRON_NAME="${CRON_NAME:-disk-io-payload-pod-error-fixed-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-disk-io-payload-pod-error-fixed}"
...
Quick start (recommended):
  ...
  # choose ONE: POD_NAME or POD_LABELS (script supports both)
  $0 set TARGET_NS=default POD_NAME=xxx
  # OR:
  # $0 set TARGET_NS=default POD_LABELS='app=stress-test'
  $0 set CONTAINER_NAME=app

  # fixed I/O error params
  $0 set IO_METHOD=read IO_ERRNO=5 IO_PERCENT=100
  $0 set IO_PATH=/data/conf

  # runtime controls
  $0 set IO_MIN_HOLD_SECONDS=120 IO_MAX_HOLD_SECONDS=300 IO_TIMEOUT_PADDING=30
  # optional:
  # $0 set IO_HOLD_SECONDS=180          # fixed hold (bypass random [min,max])

  # optional selectors
  # $0 set EVICT_COUNT=1                # or EVICT_PERCENT=50
  # $0 set WAITING_TIME=20s

  # optional
  # $0 set CLEANUP=true LOG_DIR=/logs
...
# patch env CM：./pod_disk_error_fixed_helper.sh set IO_ERRNO=5 ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
