...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/mem/high"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/mem-payload-template.pod.high.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/mem-payload-env.pod.high.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/mem-payload-script.pod.high.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/mem-payload.pod.high.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/mem-payload.pod.high.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-mem-payload-env-pod-high-configmap}"
CRON_NAME="${CRON_NAME:-mem-payload-pod-high-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-mem-payload-pod-high}"
...
Usage:
  ...
  $0 select <label-selector> [namespace] [mode]    # Choose a Pod via label selector and set it
...
Quick start (recommended):
  ...
  # Option B: automatically choose a Pod via label selector
  $0 select "app=nginx" default first
  # Or:
  $0 set TARGET_NS=default SELECTOR="app=nginx" SELECTOR_MODE=first

  # choose ONE: PERCENT or RESERVE_MB
  $0 set PERCENT=85 MAX_MEM_PERCENT=90
  # OR:
  # $0 set RESERVE_MB=512 MAX_MEM_PERCENT=90

  # runtime controls
  $0 set RAMP_SECONDS=180 MIN_HOLD_SECONDS=6000 MAX_HOLD_SECONDS=12000 TIMEOUT_PADDING=30
  # optional:
  # $0 set HOLD_SECONDS=7200            # fixed hold (bypass random [min,max])
  # $0 set TIMEOUT=600                  # hard timeout override
  # $0 set MEM_MODE=ram                 # ram / cache
  # $0 set MEM_RATE=1024                # MB/s (only for mode=ram; empty => auto calc if possible)
...
# ---------- small utils ----------
# Check whether the key already exists in the argument array (safer than grep)
has_kv_key() {
  local want="$1"; shift
  local kv
  for kv in "$@"; do
...
# Get the VALUE from a KEY=VALUE array entry
kv_get() {
  local want="$1"; shift
  local kv
  for kv in "$@"; do
    [[ "$kv" == "${want}="* ]] && { echo "${kv#*=}"; return 0; }
  done
  return 1
}

# Set/replace KEY=VALUE in the array (bash nameref)
kv_set() {
  local key="$1" val="$2" arr_name="$3"
...
# Normalize value: strip \r and trim leading/trailing whitespace (avoids "looks identical but doesn't match")
clean_val() {
  printf '%s' "$1" | tr -d '\r' | sed 's/^[[:space:]]*//;s/[[:space:]]*$//'
}

# List Pod container names (one per line)
pod_containers() {
  local ns="$1" pod="$2"
...
# Check whether the container exists in the Pod
pod_has_container() {
  local ns="$1" pod="$2" ctn="$3"
  ctn="$(clean_val "$ctn")"
  [[ -n "$ctn" ]] || return 1
  pod_containers "$ns" "$pod" | grep -Fqx -- "$ctn"
}

# Get the first container name from the Pod (fallback)
pod_first_container() {
  local ns="$1" pod="$2"
  local c
  c="$(kubectl -n "$ns" get pod "$pod" -o jsonpath='{.spec.containers[0].name}' 2>/dev/null || true)"
  clean_val "$c"
}
...
# ========= Get a Pod by label selector =========
# Arguments:
#   $1: namespace
#   $2: label selector
#   $3: selection mode (random|first)
# Returns: "pod_name:container_name" (container defaults to the first container)
get_pod_by_selector() {
  local namespace="$1"
  local selector="$2"
  local mode="${3:-random}"
  ...
  local pod_name=""
  if [[ "$mode" == "random" ]]; then
    local pod_count random_line
    pod_count="$(printf '%s\n' "$pods" | wc -l | tr -d '[:space:]')"
    [[ "${pod_count:-0}" -gt 0 ]] || { echo "ERROR: pod_count is 0" >&2; return 1; }
    random_line=$((RANDOM % pod_count + 1))
    pod_name="$(printf '%s\n' "$pods" | sed -n "${random_line}p")"
  ...
  container_name="$(pod_first_container "$namespace" "$pod_name")"

  if [[ -z "$container_name" ]]; then
    echo "ERROR: unable to get container name for Pod '$pod_name'" >&2
    return 1
  fi
...
# ========= Patch env CM: support SELECTOR auto-pick and prefer user-provided CONTAINER_NAME validation =========
cm_set() {
  local selector=""
  local target_ns=""
  local selector_mode="random"

  # Extract potential SELECTOR / SELECTOR_MODE / TARGET_NS values
  local new_args=()
  local kv
  for kv in "$@"; do
    if [[ "$kv" == SELECTOR=* ]]; then
  ...
  selector="$(clean_val "$selector")"
  selector_mode="$(clean_val "$selector_mode")"

  # If SELECTOR is present but POD_NAME is not explicit, pick a pod via the selector
  if [[ -n "$selector" ]] && ! has_kv_key POD_NAME "${new_args[@]}"; then
    ...
    pod_info="$(get_pod_by_selector "$target_ns" "$selector" "$selector_mode")" || {
      echo "ERROR: unable to select Pod with selector '$selector'" >&2
      exit 1
    }

    local pod_name container_name
    pod_name="$(clean_val "${pod_info%%:*}")"
    container_name="$(clean_val "${pod_info##*:}")"

    [[ -n "$pod_name" ]] || { echo "ERROR: empty pod_name from selector" >&2; exit 1; }

    # Add POD_NAME
    new_args+=("POD_NAME=$pod_name")

    # ---- Core logic: CONTAINER_NAME handling ----
    # 1) If the user explicitly provides CONTAINER_NAME=xxx: validate that container first;
    #    if it does not exist, fall back to the Pod's first container.
    # 2) If the user did not provide one: use the first container selected by the selector.
    if has_kv_key CONTAINER_NAME "${new_args[@]}"; then
      local want_container fallback
      want_container="$(kv_get CONTAINER_NAME "${new_args[@]}" || true)"
      want_container="$(clean_val "$want_container")"

      if [[ -n "$want_container" ]] && pod_has_container "$target_ns" "$pod_name" "$want_container"; then
        kv_set CONTAINER_NAME "$want_container" new_args
      else
        fallback="$(pod_first_container "$target_ns" "$pod_name")"
        [[ -n "$fallback" ]] || { echo "ERROR: cannot determine first container for $target_ns/$pod_name" >&2; exit 1; }
        echo "WARN: CONTAINER_NAME='$want_container' not found in $target_ns/$pod_name, fallback to '$fallback'" >&2
        kv_set CONTAINER_NAME "$fallback" new_args
      fi
    else
      kv_set CONTAINER_NAME "$container_name" new_args
    fi
  fi

  # Assemble the patch (clean values to avoid \r / whitespace issues)
  local patch='{"data":{'
  local first=1
  for kv in "${new_args[@]}"; do
    [[ "$kv" == *"="* ]] || { echo "ERROR: invalid kv: $kv (need KEY=VALUE)" >&2; exit 1; }
    local k="${kv%%=*}"
    local v="${kv#*=}"
...
cron_set() {
  [[ $# -ge 1 ]] || { echo "ERROR: cron set needs KEY=VALUE ..." >&2; exit 1; }
  ...
  for kv in "$@"; do
    ...
    case "$ku" in
      ...
      *)
        echo "ERROR: unknown cron field: $k" >&2
        echo "Allowed keys:" >&2
        echo "  schedule, suspend, concurrencyPolicy, startingDeadlineSeconds," >&2
        echo "  successfulJobsHistoryLimit, failedJobsHistoryLimit," >&2
        echo "  backoffLimit, activeDeadlineSeconds, ttlSecondsAfterFinished" >&2
        exit 1
        ;;
    esac
  done
...
job_set() {
  [[ $# -ge 1 ]] || { echo "ERROR: job set needs KEY=VALUE ... (or: job set <jobName|last> KEY=VALUE ...)" >&2; exit 1; }
  ...
  if [[ "$target" == "last" ]]; then
    jn="$(latest_job_name)"
    [[ -n "$jn" ]] || { echo "No jobs found with label app=${JOB_APP_LABEL}" >&2; exit 1; }
  else
    jn="$target"
  fi

  [[ $# -ge 1 ]] || { echo "ERROR: job set needs KEY=VALUE ..." >&2; exit 1; }
  ...
  for kv in "$@"; do
    ...
    case "$ku" in
      ...
      *)
        echo "ERROR: unknown job field: $k" >&2
        echo "Allowed keys:" >&2
        echo "  backoffLimit, activeDeadlineSeconds, ttlSecondsAfterFinished, parallelism, completions" >&2
        exit 1
        ;;
    esac
  done
...
stop_blade() {
  ...
  if [[ -z "${blade}" ]]; then
    echo "WARN: cannot read BLADE_NAME from cm/$ENV_CM_NAME" >&2
    return 0
  fi
...
# ========= select: choose a Pod via label selector and patch the ConfigMap directly =========
select_pod_by_label() {
  local selector="$1"
  local namespace="${2:-}"
  local mode="${3:-random}"
  ...
  echo "Selected Pod via selector '$selector': $pod_name (container: $container_name)"
  cm_set "TARGET_NS=$namespace" "POD_NAME=$pod_name" "CONTAINER_NAME=$container_name"
}
...
case "$cmd" in
  ...
  install)
    ...
    case "$mode" in
      ...
      *)
        echo "ERROR: install mode must be job|cron|all (or omit for cm)" >&2
        echo
        usage
        exit 1
        ;;
  ...
  cron)
    ...
    case "$sub" in
      ...
      schedule)
        expr="${1:-}"
        [[ -n "$expr" ]] || { echo "ERROR: cron schedule needs expr" >&2; echo; usage_cron; exit 1; }
        kubectl -n "$NS" patch cronjob "$CRON_NAME" --type merge -p "{\"spec\":{\"schedule\":\"$expr\"}}"
        ;;
      set)
        [[ $# -ge 1 ]] || { echo "ERROR: cron set needs KEY=VALUE ..." >&2; echo; usage_cron; exit 1; }
        cron_set "$@"
        ;;
      status)
        ...
        ;;
      *)
        echo "ERROR: cron subcmd: enable|delete|suspend|resume|schedule|set|status" >&2
        echo
        usage_cron
        exit 1
        ;;
  ...
  job)
    ...
    case "$sub" in
      ...
      delete)
        jn="${1:-}"
        [[ -n "$jn" ]] || { echo "ERROR: job delete needs <jobName>" >&2; echo; usage_job; exit 1; }
      ...
      logs)
        ...
        ;;
      set)
        [[ $# -ge 1 ]] || { echo "ERROR: job set needs KEY=VALUE ..." >&2; echo; usage_job; exit 1; }
        job_set "$@"
        ;;
      *)
        echo "ERROR: job subcmd: run|delete|status|logs|set" >&2
        echo
        usage_job
        exit 1
        ;;
  ...
  set)
    ...
    [[ $# -ge 1 ]] || { echo "ERROR: set needs KEY=VALUE ..." >&2; exit 1; }
    cm_set "$@"
    ;;
  ...
  select)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "  $0 select \"app=nginx\" default first"
      exit 0
    fi
    [[ $# -ge 1 ]] || { echo "ERROR: select needs a label selector" >&2; exit 1; }
    select_pod_by_label "$@"
    ;;
