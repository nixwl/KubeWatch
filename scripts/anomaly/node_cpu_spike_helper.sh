...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/node/cpu/spike"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/cpu-payload-template.spike.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/cpu-payload-env.spike.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/cpu-payload-script.spike.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/cpu-payload.spike.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/cpu-payload.spike.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-cpu-payload-env-spike-configmap}"
CRON_NAME="${CRON_NAME:-cpu-payload-spike}"
JOB_APP_LABEL="${JOB_APP_LABEL:-cpu-payload-spike}"
...
Install modes (split responsibilities):
  ...
  $0 uninstall               # delete CronJob + Job prerequisites + ConfigMaps (best-effort)
...
Env override:
  NS=chaosblade TEMPLATE_YAML=... ENV_YAML=... SCRIPT_YAML=... CRON_YAML=... JOB_YAML=...
...
Recommended workflow:
  ...
  $0 set TARGET_NODE=emanage1 SPIKE_PERCENT=90 SPIKE_RAMP_SECONDS=5
  $0 set SPIKE_MIN_HOLD_SECONDS=200 SPIKE_MAX_HOLD_SECONDS=300 SPIKE_TIMEOUT_PADDING=30
  ...
  # 4) install cron only (ensure configmaps already exist)
  $0 install cron
  $0 cron resume
EOF
}
...
Subcmd:
  ...
  set [last|<jobName>] KEY=VALUE   Patch a Job
  --help                           Show this help
...
# patch env CM: ./node_cpu_spike_helper.sh set TARGET_NODE=... SPIKE_PERCENT=...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
case "$cmd" in
  ...
  cron)
    ...
    case "$sub" in
      ...
      status)
        ...
        kubectl -n "$NS" get jobs --sort-by=.metadata.creationTimestamp 2>/dev/null | tail -n 20 || true
        ;;
      *)
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "Example: $0 set TARGET_NODE=emanage1 SPIKE_MIN_HOLD_SECONDS=200"
      exit 0
    fi
  ...
  show)
    ...
    kubectl -n "$NS" get cm "$ENV_CM_NAME" -o yaml 2>/dev/null | sed -n '1,200p' || echo "(no env configmap)"
    echo
