...
# ========= basic =========
DIR="/data/nfs-conf/chaosblade/job/pod/disk/delay/intermit"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/disk-io-payload-template.pod.delay.intermit.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/disk-io-payload-env.pod.delay.intermit.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/disk-io-payload-script.pod.delay.intermit.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/disk-io-payload.pod.delay.intermit.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/disk-io-payload.pod.delay.intermit.job.yaml}"

ENV_CM_NAME="${ENV_CM_NAME:-disk-io-payload-env-pod-delay-intermit-configmap}"
CRON_NAME="${CRON_NAME:-disk-io-payload-pod-delay-intermit-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-disk-io-payload-pod-delay-intermit}"
...
Quick start (recommended):
  ...
  # choose one: POD_NAME or POD_LABELS (script will remove empty matcher)
  $0 set TARGET_NS=default POD_NAME=xxx
  # or:
  # $0 set TARGET_NS=default POD_LABELS=app=stress-test POD_NAME=

  # IO delay params
  $0 set IO_METHOD=read IO_DELAY_MS=1000 IO_PERCENT=100 IO_PATH=/data/conf

  # intermit controls (choose fixed or random ranges)
  $0 set IO_MIN_SLOW_SECONDS=30 IO_MAX_SLOW_SECONDS=120
  $0 set IO_MIN_GAP_SECONDS=60 IO_MAX_GAP_SECONDS=300
  $0 set IO_MIN_CYCLES=3 IO_MAX_CYCLES=6
  $0 set IO_TIMEOUT_PADDING=30

  # optional fixed overrides:
  # $0 set IO_SLOW_SECONDS=60
  # $0 set IO_GAP_SECONDS=120
  # $0 set IO_CYCLES=4

  # optional filters:
  # $0 set EVICT_COUNT=1 EVICT_PERCENT=50 WAITING_TIME=20s
...
# patch env CM：./pod_disk_io_delay_intermit_helper.sh set IO_DELAY_MS=1000 ...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
    ...
    case "$sub" in
      ...
      status)
        ...
        kubectl -n "$NS" get jobs -l "app=${JOB_APP_LABEL}" --sort-by=.metadata.creationTimestamp -o wide 2>/dev/null \
          | tail -n 20 || true
        echo "--- recent cron child jobs (prefix: ${CRON_NAME}-) ---"
        kubectl -n "$NS" get jobs --sort-by=.metadata.creationTimestamp -o wide 2>/dev/null \
          | awk -v p="${CRON_NAME}-" 'NR==1 || index($1,p)==1' \
          | tail -n 20 || true
        ;;
      *)
