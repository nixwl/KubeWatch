...
# ========= basic =========
DIR="${DIR:-/data/nfs-conf/chaosblade/job/node/mem/spike}"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/mem-payload-template.spike.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/mem-payload-env.spike.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/mem-payload-script.spike.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/mem-payload.spike.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/mem-payload.spike.job.yaml}"

# From your spike YAMLs:
ENV_CM_NAME="${ENV_CM_NAME:-mem-payload-env-spike-configmap}"
CRON_NAME="${CRON_NAME:-mem-payload-spike-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-mem-payload-spike}"
...
Env keys (from mem-payload-env.spike.configmap.yaml):
  TARGET_NODE=<nodeName>            # target node name
  BLADE_NAME=<string>               # ChaosBlade instance name
  CLEANUP=true|false                # whether job exits cleanup CR
  PERCENT=<int>                     # target mem percent
  MAX_MEM_PERCENT=<int>             # upper clamp (default 90)
  RAMP_SECONDS=<int>                # ramp duration seconds
  MIN_HOLD_SECONDS=<int>            # hold min seconds
  MAX_HOLD_SECONDS=<int>            # hold max seconds
  TIMEOUT_PADDING=<int>             # timeout padding seconds
  HOLD_SECONDS=<int>                # optional fixed hold seconds
  MEM_MODE=ram|cache|...            # mem load mode (as supported by ChaosBlade)
...
Recommended workflow:
  ...
  $0 set TARGET_NODE=emanage1 PERCENT=85 MAX_MEM_PERCENT=90 MEM_MODE=ram RAMP_SECONDS=2
  $0 set MIN_HOLD_SECONDS=30 MAX_HOLD_SECONDS=90 TIMEOUT_PADDING=30 CLEANUP=true
  ...
  $0 install cron
  $0 cron set schedule="*/30 * * * *" activeDeadlineSeconds=600
  $0 cron resume
  $0 cron enable
EOF
}
...
# patch env CM: ./node_mem_spike_helper.sh set TARGET_NODE=... PERCENT=...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
case "$cmd" in
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "Example: $0 set TARGET_NODE=emanage1 PERCENT=85 RAMP_SECONDS=2"
      exit 0
    fi
  ...
  show)
    ...
    kubectl -n "$NS" get cm "$ENV_CM_NAME" -o yaml 2>/dev/null | sed -n '1,240p' || echo "(no env configmap)"
    echo
    echo "=== cronjob ($NS/$CRON_NAME) ==="
