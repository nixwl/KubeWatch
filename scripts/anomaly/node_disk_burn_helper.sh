...
# ========= basic =========
DIR="${DIR:-/data/nfs-conf/chaosblade/job/node/disk/burn}"
NS="${NS:-chaosblade}"

TEMPLATE_YAML="${TEMPLATE_YAML:-$DIR/disk-payload-template.burn.yaml}"
ENV_YAML="${ENV_YAML:-$DIR/disk-payload-env.burn.configmap.yaml}"
SCRIPT_YAML="${SCRIPT_YAML:-$DIR/disk-payload-script.burn.configmap.yaml}"
CRON_YAML="${CRON_YAML:-$DIR/disk-payload.burn.cron.yaml}"
JOB_YAML="${JOB_YAML:-$DIR/disk-payload.burn.job.yaml}"

# From your burn YAMLs:
ENV_CM_NAME="${ENV_CM_NAME:-disk-payload-env-burn-configmap}"
CRON_NAME="${CRON_NAME:-disk-payload-burn-cron}"
JOB_APP_LABEL="${JOB_APP_LABEL:-disk-payload-burn}"
...
Install modes (split responsibilities):
  ...
  $0 install cron            # install CronJob resources only - NO ConfigMaps/NO Job prereqs
  $0 install all             # install ConfigMaps + Job prerequisites + CronJob resources
  $0 uninstall               # delete Cron resources + Job prereqs + ConfigMaps (best-effort)
...
Ops:
  ...
  $0 stop                               # delete ChaosBlade CR by BLADE_NAME (from env configmap)
...
Env keys (from disk-payload-env.burn.configmap.yaml):
  ...
  DISK_PATH=<path>                    # required: target path on node FS (DO NOT use env var named PATH)
  READ=true|false                     # choose at least one of READ/WRITE
  WRITE=true|false
  SIZE=<uint>                         # io block size (integer), e.g. 10~50
  ...
  LOG_DIR=/logs                       # logs dir in runner (PVC mounted)
...
Recommended workflow:
  ...
  $0 set TARGET_NODE=emanage1 DISK_PATH=/var/lib/rancher READ=true WRITE=true SIZE=20
  $0 set MIN_HOLD_SECONDS=900 MAX_HOLD_SECONDS=1800 TIMEOUT_PADDING=30 CLEANUP=true
  ...
  $0 install cron
  $0 cron set schedule="0 * * * *" activeDeadlineSeconds=2000
  $0 cron resume
  $0 cron enable

Notes:
  - Job status (job status/logs) filters Jobs by label: app=${JOB_APP_LABEL} (manual runs).
  - CronJob-created Jobs are better inspected via: $0 cron status (label cronjob-name=${CRON_NAME})
EOF
}
...
# patch env CM: ./node_disk_burn_helper.sh set TARGET_NODE=... DISK_PATH=...
cm_set() {
  local patch='{"data":{'
  local first=1
  for kv in "$@"; do
...
case "$cmd" in
  ...
  install)
    ...
    case "$mode" in
      ...
      cron)
        apply_cron
        echo "CronJob resources installed. (No ConfigMaps, no Job prerequisites.)"
        ;;
      all)
        apply_configmaps
        apply_job_prereqs
        apply_cron
        echo "All installed: ConfigMaps + Job prerequisites + CronJob resources."
        ;;
      *)
  ...
  set)
    if [[ "${1:-}" == "-h" || "${1:-}" == "--help" ]]; then
      ...
      echo "Example: $0 set TARGET_NODE=emanage1 DISK_PATH=/var/lib/rancher READ=true WRITE=true SIZE=20"
      exit 0
    fi
